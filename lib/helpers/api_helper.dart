...
class APIHelper extends ChangeNotifier {
  ...
  static const BASE_URL =
      "mobileolx.srkwebappinnovations.com"; // Put Your Base Url here.
  // static const BASE_URL = "mobileolx.srkwebappinnovations.com/index.php?action="; // Put Your Base Url here.
  ...
  Future fetchProducts() async {
    final String url =
        'https://$BASE_URL/$UNENCODED_PATH?action=$PRODUCT_LIST_URL';
    ...
    print(
        "Fetch Products+++++++++++++++++++++++++++++++++++++++++++++++++++==============================$decodeData");
    print(decodeData);
    List<MainProduct> products = [];

    for (int i = 0; i < decodeData.length; i++) {
      try {
        products.add(MainProduct.fromJson(decodeData[i]));
      } catch (e) {
        print(e);
      }
    }
    ...
    try {
      for (var i = 0; i < productImages.length; i++) {
        if (productImages[i].file != null && productImages[i].isLocal) {
          ...
          print(
              "Here is the response i recived after uploading the picture=================+++++====++++++++++++++++++++${response}");
          ...
          if (response.statusCode == 200) {
            ...
            print(
                "Server decodedResponse i get======================================${decodedResponse}");
          }
          print(
              "here is response i get======================================${response.statusCode}");
        }
      }

      print(
          "Here is url of image ==============================${serverImagesList[0]}");
      ...
      http.Response response2 = await http.post(url);
      print(
          "Here is the response comes fromt he api================================${response2.statusCode}");
      if (response2.statusCode == 200) {
        final decodedData = await json.decode(response2.body);
        return decodedData;
      }
    } catch (e) {
      print("I have faced this issue -------------------------------------$e");
      throw e.toString();
    }
  }
